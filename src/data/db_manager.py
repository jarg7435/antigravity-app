...
class DataManager:
    ...
    def get_last_match_lineup(self, team_name: str) -> List[str]:
        """
        Semanlly searches for the most recent match of a team and returns its lineup.
        Used as a reference/base lineup if current source data is unavailable.
        """
        if self.use_supabase:
            # Search for matches where the team played (home or away)
            row = self._sb_get("matches", 
                f"or=(home_team.eq.{requests.utils.quote(team_name)},away_team.eq.{requests.utils.quote(team_name)})"
                f"&select=data_json&order=date.desc&limit=1")
            if row:
                match = Match.model_validate_json(row[0]["data_json"])
                if match.home_team.name == team_name:
                    return [p.name for p in match.home_team.players]
                else:
                    return [p.name for p in match.away_team.players]
        else:
            conn = sqlite3.connect(self.db_path)
            r = conn.execute('''
            ...
            conn.close()
            if r:
                match = Match.model_validate_json(r[0])
                if match.home_team.name == team_name:
                    return [p.name for p in match.home_team.players]
                else:
                    return [p.name for p in match.away_team.players]
